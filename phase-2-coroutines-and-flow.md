...
### 4. How it works
A `suspend` function achieves main-safety by using `withContext(Dispatcher.IO)` or similar internally to shift its own execution off the main thread before doing heavy work.
...
```kotlin
// MISTAKE: This SupervisorJob is the parent of the launch, NOT the launch's internal job.
// It DOES NOT do what you think! Siblings will still cancel.

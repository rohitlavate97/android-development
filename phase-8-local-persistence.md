...
- *Mid:* "Why did Google deprecate SharedPreferences in favor of DataStore?"
...
### 6. Production usage
Managing user sessions, feature flags, UI toggle states (like dark mode, list vs. grid view), and storing simple authentication tokens securely (often combined with Encrypted DataStore).
...
@Database(
    ...
    version = 2,
    // Option A: AutoMigration (Preferred for simple additions)
    autoMigrations = [AutoMigration(from = 1, to = 2)]
)
abstract class AppDatabase : RoomDatabase()

// Building the DB with manual migration

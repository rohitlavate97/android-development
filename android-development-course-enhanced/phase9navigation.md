...
### 9. Testing
Unit testing ViewModels becomes trivial. There is no special "route-aware" `SavedStateHandle` factory — `toRoute<T>()` just deserializes whatever key/value pairs are already in the handle's backing map, matching them to your route class's constructor parameter names. So in a test you construct the handle directly with the same keys your route class declares:
```kotlin
val savedStateHandle = SavedStateHandle(mapOf("expenseId" to "123"))
val viewModel = ExpenseDetailViewModel(savedStateHandle, fakeRepository)
// viewModel.expenseId == "123"
```
No navigation-testing artifact or `NavController` is needed at all — this is a plain unit test.
...
### 2. Why does it exist
1. **TransactionTooLargeException**: The Binder IPC layer that carries `Bundle`s under the hood (used for navigation state, process death restoration, and inter-process calls generally) has a transaction buffer of about 1MB **shared by every in-flight transaction for the whole process**, not a per-call allowance. In practice you can hit `TransactionTooLargeException` well before 1MB if other transactions are competing for that buffer at the same time, so treat anything above a few tens of KB as risky. Passing large objects crashes the app.
...
### 4. How it works
Replace `composable<Route>` with `dialog<Route>` (built-in to `androidx.navigation:navigation-compose`). When navigated to, it renders over the previous back stack entry without destroying it.

> **[Extension] Note:** There is no first-party `bottomSheet<Route>` builder in the current `navigation-compose` artifact. Google's old `accompanist-navigation-material` library provided one, but Accompanist has been archived/sunset and is not a safe recommendation for new production code. As of this writing, teams that want a bottom sheet as a real back-stack destination either (a) wrap a Material 3 `ModalBottomSheet` inside a `dialog<Route>` destination (simplest, fully first-party, works today), or (b) pull in a small third-party library purpose-built for this gap (e.g. `compose-material3-navigation`). Don't assume a `bottomSheet()` DSL function ships out of the box — check what's actually on your version of `navigation-compose` before writing the import.
...
### 6. Production usage
In the Expense Tracker, tapping a date in a chart opens a Bottom Sheet showing expenses for that day. It is routed via `dialog<DailySummaryRoute>` with a Material 3 `ModalBottomSheet` as the dialog's content (see the `[Extension]` note above). Swiping it away or pressing Back pops the stack naturally.
...
> **[Extension] Predictive Back testing gotchas:** A few things trip teams up when they actually turn Predictive Back on rather than just setting the manifest flag:
> - **It requires `targetSdkVersion 34`+ as well as the manifest flag.** Setting `android:enableOnBackInvokedCallback="true"` with a lower `targetSdk` compiles fine and does nothing — the system silently falls back to the legacy dispatcher.
> - **`BackHandler { }` still works, but it does not get the predictive peek animation.** To get the interactive swipe-and-peek UI (rather than an all-or-nothing back press), you must use `PredictiveBackHandler`, which hands you a `Flow` of `BackEventCompat` progress updates (start → progress 0..1 → commit/cancel) that you use to drive your own preview transform.
> - **Espresso/UI-Automator cannot drive a partial predictive-back gesture.** Standard instrumented tests only exercise the fully-committed back press, so the interactive peek/cancel path is effectively untestable outside manual QA and emulator gesture recording — plan for that gap rather than assuming CI will catch a broken half-swipe.
> - **Custom `AnimatedContentTransitionScope` transitions must be seekable** (built from `tween`/`spring` specs Compose can scrub, not one-shot side effects) or the predictive preview will jump instead of tracking the user's finger.

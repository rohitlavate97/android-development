...
> **[Extension] Note:** The "unscoped by default" framing is precisely true for Dagger/Hilt (omit any scope annotation and you get a new instance per injection). Koin doesn't actually have an implicit default — every declaration in a Koin module must explicitly say `single { ... }` / `singleOf(...)` (singleton) or `factory { ... }` / `factoryOf(...)` (new instance per injection); there is no annotation-free fallback the way Dagger has. So when comparing the two frameworks, it's more accurate to say Koin has "no default, mandatory explicit choice" rather than "unscoped by default" — the practical effect is similar (you won't accidentally get a shared singleton), but the mechanism differs.
...
*   **Q:** What is the default scope in Dagger/Hilt or Koin?
    *   **A:** In Dagger/Hilt, unscoped (a new instance is created on every injection) whenever you omit a scope annotation. Koin has no implicit default at all — every `module { }` entry must explicitly declare `single { }`/`singleOf(...)` or `factory { }`/`factoryOf(...)`, so "forgetting" a scope in Koin is a compile error on the DSL, not a silent unscoped fallback.
...
> **[Extension] KSP over KAPT:** Hilt has supported the Kotlin Symbol Processing (KSP) compiler plugin as an alternative to KAPT for several years now, and KSP is the recommended default going forward — it processes Kotlin symbols directly instead of generating a Java stub first, which is measurably faster and produces better IDE error locations. If you're setting up a new project, prefer `ksp("com.google.dagger:hilt-compiler:...")` over the KAPT equivalent; only fall back to KAPT if another annotation processor in your build graph doesn't yet support KSP.
>
> **[Extension] Injecting into non-Hilt-managed classes:** `@AndroidEntryPoint` only works on Activity, Fragment, View, Service, and BroadcastReceiver — classes the Android OS itself instantiates through a route Hilt can intercept. For classes Hilt does *not* control the construction of (a `ContentProvider`, a plain `Worker` you don't want to make a `HiltWorker`, or a static utility invoked from Java-only code), use an `@EntryPoint` interface annotated with `@InstallIn` and retrieve it via `EntryPointAccessors.fromApplication(context, MyEntryPoint::class.java)`. Forgetting this and trying to slap `@Inject` directly on a `ContentProvider` field is a common "why isn't Hilt injecting this?" bug report.
...
> **[Extension] `@BindValue` for lighter-weight test doubles:** Writing a full `@Module` + `@TestInstallIn` pair just to swap in one fake is heavy for a single test class. Hilt's `hilt-android-testing` artifact also provides the `@BindValue` field annotation: inside a `@HiltAndroidTest`-annotated test class, annotate a field holding your fake/mock with `@BindValue`, and Hilt automatically binds that field's runtime value into the graph for that test — no separate module file needed. Reserve full `@TestInstallIn` modules for bindings that need to be swapped across an entire test suite; use `@BindValue` for one-off, per-test-class fakes.

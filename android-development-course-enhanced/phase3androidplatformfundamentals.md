...
> [Extension] Note: Single-Activity architecture intersects with the **predictive back gesture** the OS introduced (opt-in from Android 13, default-on behavior for apps targeting API 35+). Because there's only one Activity, the system can no longer infer "back" as "finish the Activity" between your screens — your app is responsible for intercepting back navigation per-screen. In Compose, this means registering a `PredictiveBackHandler`/`BackHandler` (from `androidx.activity.compose`) around your `NavHost` so in-app back stack navigation animates and behaves correctly, and declaring `android:enableOnBackInvokedCallback="true"` in the manifest. Skipping this is a common source of "back button does nothing" or "back exits the whole app" bugs in single-Activity apps.
...
- **[Extension] Missing foreground service type:** If a `Worker` needs to run long enough (or urgently enough) that you promote it via `setForeground()`/expedited work, Android 14 (API 34) requires you to declare an explicit foreground service `type` (e.g., `dataSync`) in the Manifest and request the matching permission. Enqueuing expedited/foreground work without this on a device targeting API 34+ throws a `MissingForegroundServiceTypeException` at runtime, not a compile error — worth knowing before you ship.

...
> [Extension] Note: The K2 compiler (Kotlin's default compiler frontend since Kotlin 2.0) noticeably widened smart-cast coverage — it can now smart-cast a `var` captured in a local lambda, and properties whose getter is derived from a local `val`, in more cases than the old frontend could. In practice this means fewer forced `!!` or local-copy workarounds than you may have needed in older Kotlin versions; if you're reading pre-2.0 code (or Stack Overflow answers) that routes around a "smart cast impossible" error, double-check it's still necessary on your current compiler version.
...
* `with`: [Extension] The odd one out — it's not an extension function on the receiver, it's a regular top-level function that *takes* the object as an argument: `with(receiver) { ... }`. Same (`this`, block result) behavior as `run`, but you can't chain it with `?.` off a nullable value.
...
> [Extension] The heading for this section names all five scope functions, but the original content below only walked through `apply`, `let`, and `also` in code/mistakes/exercises. `with` is filled in here since it's part of the promised set and shows up constantly in production code (e.g. `with(binding) { ... }` in XML/View code, or grouping several reads off one object).
...
// --- WITH: Grouping calls on a non-nullable receiver --- [Extension]
// Context: 'this'. Returns: Result of block. NOT an extension function (takes receiver as a parameter).
val summary = with(newExpense) {
...
* `with` [Extension] is used when you have a known non-null object and want to make several calls/reads on it without repeating its name (e.g. `with(binding) { title.text = ...; subtitle.text = ... }` in classic View-based UI code).
...
> [Extension] Note: "Zero overhead" is the common shorthand, but it's true only when the compiler can inline the value class away — i.e., when it's used as a concrete, non-nullable type in a direct function signature. If you box it (assign to a nullable type, store it in a generic collection like `List<AccountId>`, or pass it as a type argument), the compiler falls back to allocating a real wrapper object, same as a `data class`. For hot paths, don't assume zero-cost without checking where the value class actually ends up.

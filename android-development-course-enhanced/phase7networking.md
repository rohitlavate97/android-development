...
### 11. Deliberate failure
Remove the `suspend` keyword from `getExpenses` but leave its return type as `List<ExpenseDto>` (not `Call<List<ExpenseDto>>`). Try to build/call it. Retrofit does **not** silently fall back to running the call on whatever thread invoked it — instead it throws `IllegalArgumentException: Unable to create call adapter for ...` (either eagerly at `retrofit.create()` time if you set `.validateEagerly(true)`, or the first time you invoke that method), because a non-`suspend` method must return a `Call<T>` (or a type another registered `CallAdapter.Factory`, like RxJava's, understands) — Retrofit has no way to run a bare `List<ExpenseDto>`-returning method asynchronously otherwise. *(The classic `NetworkOnMainThreadException` gotcha is a related but different mistake: keeping the return type as `Call<List<ExpenseDto>>` and then calling `.execute()` — the synchronous variant — directly from the main thread instead of `.enqueue()` or a `suspend` function.)*
...
> **[Extension] Choosing a Ktor engine:** The `HttpClient(OkHttp) { ... }` constructor call explicitly wires the Android/JVM `OkHttp` engine (bundling OkHttp's connection pooling, HTTP/2, and interceptor support underneath Ktor's DSL). Ktor also ships a pure-Kotlin `CIO` (Coroutine-based I/O) engine that has zero Java dependencies, which matters for KMP targets without a JVM (e.g. `Darwin` is the equivalent native engine on iOS/macOS, `Js`/`WinHttp` elsewhere). For an Android-only or Android+iOS-shared module, prefer the `OkHttp` engine on the Android side specifically so you can reuse your existing `Interceptor`/`Authenticator` token-refresh logic — dropping to `CIO` on Android means re-implementing that logic against Ktor's own `HttpClientPlugin` API instead.
...
> **[Extension] Forgetting a retry-count guard.** There's a second, subtler infinite-loop risk this example doesn't show: if the refresh call *succeeds* but the server rejects the brand-new access token too (a backend bug, a clock-skew issue, or a revoked refresh token that still returns a stale-looking success), OkHttp will call `authenticate()` again with the new failed response — forever, once per retry. Production `Authenticator` implementations should always cap retries by walking the `priorResponse` chain, e.g. `generateSequence(response) { it.priorResponse }.count() >= 3 -> return null` (giving up and forcing a real logout) before attempting another refresh. Without this guard, a single bad token can put the app into a silent, battery-draining refresh loop instead of surfacing a login screen.

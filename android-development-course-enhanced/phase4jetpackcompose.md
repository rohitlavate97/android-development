...
> [Extension] If you ever need to *build* a custom modifier (not just chain existing ones), know that the modern, performant way is implementing the `Modifier.Node` API (`ModifierNodeElement` + a `Modifier.Node` subclass) rather than the older `composed { ... }` factory. `composed {}` creates a new composition scope per usage (extra allocation, harder to reason about) and is now considered legacy for anything beyond quick prototypes; `Modifier.Node` avoids that overhead and is what Compose's own built-in modifiers (`clickable`, `padding`, etc.) are implemented with.
...
> [Extension] Note: The Compose compiler's "Strong Skipping Mode" has been the default since Compose compiler 1.5.4+ (bundled by default in current Android Gradle Plugin/Compose BOM releases). It lets Compose skip a composable even when one of its parameters is *unstable* (like a plain `List<T>`), by comparing the unstable argument with instance equality (`===`) against its last-seen value instead of refusing to track it at all. This significantly reduces — but does not eliminate — the practical need for `ImmutableList`/`@Immutable` wrappers: it still won't help if you pass a *new* `List` instance built from the same data on every recomposition (a fresh `.filter{}`/`.map{}` call, for example), since that new instance fails the `===` check. The stability techniques taught above are still correct and still matter for that common case — just know that "unstable parameter" no longer automatically means "never skippable" on current tooling.

        ...
        appUpdateInfoTask.addOnSuccessListener { appUpdateInfo ->
            if (appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE
                ...
                appUpdateManager.startUpdateFlowForResult(
                    appUpdateInfo,
                    AppUpdateType.IMMEDIATE, // Blocks the UI until updated
                    this,
                    UPDATE_REQUEST_CODE
                )
            }
...
### 4. How it works
You cannot "rollback" an app on a user's phone. Once they install v2.0, you cannot force their phone to downgrade to v1.9. Therefore, mobile rollbacks are actually "Roll Forwards" — you release v2.0.1 containing the fix or reverting the feature.
...
**Requirements:**
1. **Security Layer:**
   - Implement `SecureStorageRepository` using `EncryptedSharedPreferences` backed by Android Keystore `MasterKey`.
...
|---|---|---|---|
| 1. The Language | Null safety, Extension functions, Data classes | Pure Kotlin Domain Models | From verbose POJOs & boilerplate to concise, safe state representation |
| 2. Collections & FPs | `map`, `filter`, `fold`, Sequences | Data Transformation Pipelines | From imperative `for` loops to functional streams |
| 3. Concurrency | Coroutines, `suspend`, Dispatchers | Async API fetching | From Thread/RxJava callbacks to structured sequential async code |
| 4. Architecture | MVVM, UDF, StateFlow | ViewModels handling UI State | From spaghetti UI manipulation to predictable state machines |
| 5. Dependency Inj. | Hilt / Dagger, Scopes, Modules | DI Graph for repositories | From `new Object()` coupling to interface-driven testable injection |
| 6. UI Layer | Jetpack Compose, Recomposition | Declarative UI Screens | From XML layouts & `findViewById` to reactive, code-driven UI |
| 7. Local Data | Room Database, Flow observations | Offline-first persistence layer | From raw SQLite helpers to reactive ORM data streams |
| 8. Networking | Retrofit, OkHttp, Moshi/Serialization | REST API Clients | From raw HttpUrlConnection to type-safe API interfaces |
| 9. Background | WorkManager, Constraints | Sync Jobs | From AlarmManager to guaranteed deferred execution |
| 10. Navigation | Jetpack Navigation Compose, Deep Links | App routing | From manual `Intent` passing to type-safe graph navigation |
| 11. Testing | JUnit5, MockK, Coroutine Test Dispatchers | Unit & Integration Test Suites | From QA Blackbox testing to developer-driven Whitebox coverage |
| 12. App Quality | Profiler, LeakCanary, R8 | Optimized Release Build | From unoptimized debug builds to memory-safe, obfuscated artifacts |
| 13. Release & Sec | Keystore, Fastlane, Feature Flags | CI/CD Pipeline & Incident Playbook | From manual file signing to automated, safe deployment processes |
